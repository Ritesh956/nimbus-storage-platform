...
- **Distributed tracing** (roadmap #14): `internal/platform/tracing` wraps the OpenTelemetry SDK, exporting via OTLP/HTTP to Grafana Tempo (`NIMBUS_OTEL_EXPORTER_ENDPOINT`, optional — empty leaves the SDK's own no-op provider in place, same graceful-degradation shape as `NIMBUS_SMTP_ADDR`). `httpserver.Tracing` starts a span per HTTP request, named by mux pattern like the metrics middleware; the span context is injected into outgoing NATS message headers (`internal/events`) and extracted back out by `events.Subscribe`, so a request's trace continues into `nimbus-worker`'s processing as child spans (`reassemble_chunks`/`generate_thumbnail`/`store_thumbnail`, `internal/processing`) instead of starting a disconnected one — the audit's "upload-complete → thumbnail-generated, demoable in one screenshot" ask. Live-verified via Tempo's own search/trace API: a real upload's trace tree showed exactly this parent/child shape spanning both processes.

## 3. Deployment topology

**Local dev (Docker Compose) — built and running.** `deploy/docker-compose.yml`: `nimbus-api`, `nimbus-worker`, `nimbus-web`, `postgres`, `redis`, `nats`, `minio-node-1/2/3` (standalone, not clustered), `prometheus`, `grafana`, `tempo` (trace backend, roadmap #14). This is the primary day-to-day loop for the backend. Grafana moved to host port 3001 (was 3000) when `nimbus-web` was added Day 13 — 3000 is the frontend's port, matching `npm run dev` and the README.
...
- `infra/` is the lightweight-manifests half — Postgres and the three MinIO nodes as single-replica StatefulSets (deliberately three independent ones, not one 3-replica StatefulSet, for the same reason Compose runs three standalone MinIO containers rather than MinIO's own distributed mode: nimbus's own placement/replication logic is the point). Redis and NATS are plain Deployments, unvolumed, matching Compose. Prometheus/Grafana/Tempo read the *same* `deploy/observability/` files Compose does (`apply.sh` generates their ConfigMaps via `kubectl create configmap --from-file` rather than duplicating the content). Tempo (roadmap #14) is ClusterIP-only, no NodePort — it's queried in-cluster by Grafana's Tempo datasource and receives OTLP in-cluster from `nimbus-api`/`nimbus-worker`, nothing outside the cluster needs to reach it directly.
...
- **Share**: `sharing.CreateShare`/`CreateFolderShare`/`CreateBundleShare` persist a token+expiry scoped to one file, a folder subtree, or a picked file set (migration 000009, post-Tier-3 session); unauthenticated `sharing.Resolve` re-enters the download lifecycle above via public-safe `FileInfo`/`FolderInfo` projections instead of a user session, with folder/bundle items fetching per-file plans on demand through token-scoped public endpoints.
...
- **Async processing**: `nimbus-worker` consumes `nimbus.uploads.completed` (durable JetStream consumer, max 5 redeliveries) → `processing.Processor.Process` reassembles chunks → generates a real thumbnail — images via stdlib decode+scale; PDFs by rendering the first page with pdfium-in-WASM (no cgo, warmed up at worker boot; the old solid-color placeholder survives only as the fallback for unparseable PDFs — post-Tier-3 session) → stores it → `file.SetThumbnailKey` → best-effort `activity.RecordThumbnail`.
