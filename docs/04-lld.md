...
The rate limiter (built in the Tier 2 backlog session — `internal/platform/ratelimit`) is a Redis token bucket run as a Lua script (atomic refill+spend, shared across api replicas). Bucket key: user ID when the request carries a signature-valid access token (`auth.Service.PeekUserID` — signature check only, no blacklist round-trip; real authorization still happens in authMiddleware), else client IP. Defaults 25 rps / burst 50 (`NIMBUS_RATE_LIMIT_RPS`/`_BURST`, RPS 0 disables). `/healthz`, `/readyz`, `/metrics` are exempt — fixed-interval pollers must not eat a caller's budget. Redis failure fails open: degraded abuse protection beats turning a Redis blip into an API outage. **Corrected in the pre-deployment audit session**: `ratelimit.ClientIP` reads the *last* `X-Forwarded-For` hop, not the first — the first hop is attacker-controlled (a client can send a fresh fake value every request), while the last hop is what a single trusted reverse proxy (the go-public plan's Caddy) actually appends.

**Dedicated login bucket (audit roadmap item #6, quick-wins session)**: `POST /v1/auth/login` and `POST /v1/auth/login/totp` run before any access token exists, so the general per-user bucket above never applies to them — and its generous default was cheap enough to make online password/TOTP guessing against a single account viable. A second, separate `ratelimit.Limiter` wraps just those two routes, keyed by IP alone (`"login-ip:" + ClientIP`, never falling back to a user-ID key since none exists yet), defaulting to 1 rps / burst 5 (`NIMBUS_LOGIN_RATE_LIMIT_RPS`/`_BURST`, RPS 0 disables and leaves only the general limiter, if any, covering these routes).
