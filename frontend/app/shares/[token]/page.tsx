  ...
  return (
    <div className="flex flex-1 items-center justify-center px-4">
      <Card className="w-full max-w-md text-center">
        ...
        {!error && !resolved && <p className="mt-4 text-muted">Loading…</p>}
        {resolved && (
          <>
            <h1 className="mt-2 text-xl font-semibold break-all">{resolved.file.name}</h1>
            <p className="mt-1 text-sm text-muted">
              {formatBytes(resolved.file.size_bytes)} · {resolved.file.mime_type}
            </p>
            <Button className="mt-6 w-full" disabled={downloading} onClick={download}>
