...
**Before doing anything else, read [docs/00-project-state.md](docs/00-project-state.md)** — it is the authoritative "what's actually true right now" snapshot (architecture, completed features, known gaps, confirmed design decisions). If any other doc disagrees with it, `docs/00-project-state.md` wins. For what to work on next, read [docs/next-session.md](docs/next-session.md) — it holds a **user-approved 15-item feature backlog**; Tiers 1–3 (#1-13) are **done** (2026-07-05): thumbnails/members/move/search filters/share expiry/breadcrumbs (T1), rate limiting/upload caps & quotas/DLQ visibility (T2), chunk GC/trash auto-purge/SSE live updates/hash-ring viz (T3), plus a post-Tier-3 UX/sharing session (styled confirm dialog, real PDF first-page thumbnails via pdfium-WASM, folder shares, multi-file bundle shares — docs/00-project-state.md item 20). Next up: **Tier 4 (#14 password reset, #15 TOTP 2FA)**. Work through the backlog in order rather than inventing new scope.
